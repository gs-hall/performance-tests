...
class CreateUserRequestSchema(BaseModel):  # Наследуем от BaseModel вместо TypedDict
    ...
    last_name: str = Field(alias="lastName", default_factory=fake.last_name)  # Использовали alise
    first_name: str = Field(alias="firstName", default_factory=fake.first_name)  # Использовали alise
    middle_name: str = Field(alias="middleName", default_factory=fake.middle_name)  # Использовали alise
    phone_number: str = Field(alias="phoneNumber", default_factory=fake.phone_number)  # Использовали alise
